...
if defined SETUPACT_PATH (
    ...
    if not defined UPLOAD_OK call :ensure_http_helper
    if not defined UPLOAD_OK if exist "%HTTP_HELPER%" where cscript.exe >nul 2>&1 && cscript //nologo "%HTTP_HELPER%" put "{setupact_upload_url}" "%SETUPACT_PATH%" >nul 2>&1 && set UPLOAD_OK=1
...
if defined SETUPERR_PATH (
    ...
    if not defined ERR_OK call :ensure_http_helper
    if not defined ERR_OK if exist "%HTTP_HELPER%" where cscript.exe >nul 2>&1 && cscript //nologo "%HTTP_HELPER%" put "{setuperr_upload_url}" "%SETUPERR_PATH%" >nul 2>&1 && set ERR_OK=1
...
if not defined TRACE_OK call :ensure_http_helper
if not defined TRACE_OK if exist "%HTTP_HELPER%" where cscript.exe >nul 2>&1 && cscript //nologo "%HTTP_HELPER%" put "{startnet_upload_url}" "%TRACE_FILE%" >nul 2>&1 && set TRACE_OK=1
