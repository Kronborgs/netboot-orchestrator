...
call :u
call :t
rem Disable Windows Filtering Platform (WFP/WFPLWF) in WinPE BEFORE setup.exe.
rem On Win10/11 22H2+, WFP binds to the NIC and blocks setup.exe from marking it
rem as boot-critical, causing 'a required driver could not be installed'.
wpeutil DisableFirewall >nul 2>&1
set WFP_RC=!errorlevel!
call :trace wpeutil DisableFirewall rc=!WFP_RC!
call :log_http "{log_url_base}disablefirewall_rc_!WFP_RC!"
